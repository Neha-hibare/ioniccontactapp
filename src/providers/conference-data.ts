...
export class ConferenceData {
  ...
  load(): any {
   ...
   processUserData(data: any) {
    ...
    this.data.userData.forEach((city: any) => {
      // loop through each timeline group in the day
      city.areaList.forEach((area: any) => {
        // loop through each session in the timeline group
        area.users.forEach((user: any) => {
          user.contacts = [];
         // user.address = [];
         // if (user.id) {
           console.log("user.id ---- "+user.id);
              let person = this.data.contacts.find((s: any) => s.id === user.id);
              console.log("person ----- "+person);
              if (person) {
                user.contacts.push(person);
